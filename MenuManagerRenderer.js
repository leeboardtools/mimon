 ...
 * <li>The menu template is passed via {@link ipcRenderer#send} to the main process, 
 * where the corresponding main process {@link MenuManagerMain} has been instantiated.
 * <li>{@link MenuManagerMain} handles the actual creation of an Electron 
 * [Menu]{@link https://electronjs.org/docs/api/menu} object
 * fills in click functions, and installs the menu as the application menu.
 * <li>{@link MenuManager#setMenuTemplate} scans the template looking for menu item 
 * entries that have an <code>idRenderer</code> property. These menu items are the 
 * ones handled by the menu manager. These items are also set disabled.
 * <li>When someone handles a particular menu item id, it calls 
 * {@link MenuManager#on} with the appropriate
 * <code>idRenderer</code> and passing in a listener. When this is done the manager 
 * sends an IPC message to the main process to enable the menu items with the id.
 * <li>When someone no longer handles a particular menu item id, it calls 
 * {@link MenuManager#off} with the
 * <code>idRenderer</code> and listener that were passed to {@link MenuManager#on}. 
 * When this is done and there are no more listeners for a particular menu item id, 
 * the manager sends an IPC message to the main process to disable the menu items with 
 * the id.
 * <li>When a menu item with an <code>idRenderer</code> is chosen, the main processes 
 * sends an IPC message back to this manager, which then calls the listeners 
 * associated with the id. Listeners are called from the newest one passed to 
 * {@link MenuManager#on}.
 * If a listener handles the message is needs to return <code>true</code> or some 
 * other truthy value, otherwise the next listener will be called.
 */
export class MenuManager {
    ...
    /**
     * Installs a menu template. Menu items to be handled by the menu manager on the 
     * renderer side of things should have a <code>idRenderer</code> property.
     ...
     * @returns {boolean}   A truthy value if the id was processed and should not be 
     * processed further.
     */
     ...
     * @param {boolean} [onlyIfUnique=false]    If <code>true<code> only one instance 
     * of listener will be added, otherwise multiple instances may be added if this 
     * is called multiple times with the same listener. This is useful when listeners 
     * are being installed/removed based upon some state other than the active 
     * UI component.
     */
     ...
     * This is useful when listeners are being installed/removed based upon some 
     * state other than the active UI component.
     * @param {string} id
     * @param {MenuManager~ClickListener} listener
     */
    ...
    /**
     * Helper that calls either {@link MenuManager#on} or {@link MenuManager#off} 
     * depending upon the condition arg. This is useful for enabling/disabling menu 
     * items based on a boolean value.
    ...
    /**
     * Enables/disabling the overriding of a menu item set to a role. When enabled, 
     * the menu item is treated as a normal menu item, when disabled the role 
     * originally assigned to the menu item's template is installed.
     * @param {string} id
