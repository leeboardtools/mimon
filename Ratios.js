...
/**
 * Represents a ratio using a numerator and a denominator, tracking the actual 
 * numerator and denominator values as the ratio is multiplied or divided by other 
 * ratios.
 */
export class Ratio {
    /**
     ...
     * <li>(number, [number=1]) Where the first number is the numerator and the 
     * second number, if given, is the denominator.
     * <li>(number[], number[]) Where the first arg is an array of the numerator 
     * values and the second arg is an array of the denominator values. The array 
     * sizes must be the same.
     * <li>(Ratio[]) An array of Ratio objects.
     * <li>(...Ratio) One or more Ratio objects.
     * <li>({@link Ratio~JSON}) An object with the same format as generated by 
     * {@link Ratio#toJSON}.
     * @param  {...any} args 
     */
    constructor(...args) {
    ...
    /**
     * Returns a new {@link Ratio} whose numerators are a combination of the 
     * numerators of this and another Ratio's numerators, and whose denominators 
     * are a combination of the denominators of this and another Ratio's denominators.
     * @param {Ratio} other 
     */
    multiply(other) { 
        return new Ratio(this._numerators.concat(other._numerators), 
            this._denominators.concat(other._denominators)); 
    }

    /**
     * Returns a new {@link Ratio} whose numerators are a combination of the 
     * numerators of this and another Ratio's denominators, and whose denominators 
     * are a combination of the denominators of this and another Ratio's numerators.
     * Essentially the same as this.mulitiply(other.inverse());
     * @param {Ratio} other 
     */
    divide(other) { 
        return new Ratio(this._numerators.concat(other._denominators), 
            this._denominators.concat(other._numerators)); 
    }

    /**
     * Returns an array containing the reduced numerator and denominator of this.
     * @returns {number[]}  An array whose first element is the reduced numerator 
     * and whose second element is the reduced denominator.
     */
    getReducedNumeratorDenominator() {
        ...
        // we just have to go through the denominator primes and subtract their 
        // powers from the numerator prime powers.
    ...
    /**
     * Returns a new {@link Ratio} whose numerators and denominators consist of 
     * only one value.
     * @returns {Ratio}
     */
    reduce() {
...
/**
 * Retrieves a {@link Ratio} representing a given arg, returning the arg if it 
 * is in fact a {@link Ratio}.
 * @param {*} ratio 
...
/**
 * Retrieves a {@link Ratio~JSON} representing a given arg, calling 
 * {@link Ratio#toJSON} if the arg is a {@link Ratio},
