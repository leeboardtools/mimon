...
test('LotStates-ROC', () => {
    ...
    result = L.createROCLotChangeDataItems(statesA, 150000);
    ...
    const statesB = [
        ...
        },
        {
            ...
            costBasisBaseValue: 400000,
        },
        {
            lotId: 3,
            quantityBaseValue: 500000,
            costBasisBaseValue: 75000,
        }
    ];

    let quantityBaseValueB = 0;
    statesB.forEach((state) => quantityBaseValueB += state.quantityBaseValue);

    const rocB = 300000;
    const changeB1 = -Math.round(rocB 
        * statesB[0].quantityBaseValue / quantityBaseValueB);
    const changeB2 = -Math.round(rocB 
        * statesB[1].quantityBaseValue / quantityBaseValueB);
    const changeB3 = -rocB - changeB1 - changeB2;

    result = L.createROCLotChangeDataItems(statesB, rocB);

    expect(result.lotChanges).toEqual([
        {
            lotId: 1,
            costBasisBaseValue: changeB1,
        },
        {
            lotId: 2,
            costBasisBaseValue: changeB2,
        },
        {
            lotId: 3,
    ...
    //
    // To Test: Capital Gains...
    const statesC = [
        {
            lotId: 1,
            quantityBaseValue: 1000000, // 100/400 = 1/4
            costBasisBaseValue: 10000,
        },
        {
            lotId: 2,
            quantityBaseValue: 500000,  // 50/400 = 1/8
            costBasisBaseValue: 500,
        },
        {
            lotId: 3,
            quantityBaseValue: 2500000, // 250/400 = 5/8
            costBasisBaseValue: 200000,
        }
    ];

    //const lotQuantityBaseValueC = 4000000;
    const rocC = 8000;
    const changeC1 = -(rocC / 4);    // 1/4
    const changeC2 = -500; // Need to drop to 0.
    const changeC3 = -(rocC * 5 / 8);

    result = L.createROCLotChangeDataItems(statesC, rocC);

    expect(result.lotChanges).toEqual([
        {
            lotId: 1,
            costBasisBaseValue: changeC1,
        },
        {
            lotId: 2,
            costBasisBaseValue: changeC2,
        },
        {
            lotId: 3,
