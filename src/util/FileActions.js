...
/**
 * Replaces the base name of a path (name.ext) with another base name or the base name from another path.
...
/**
 * Retrieves a path name given a file name that may just be a base name (name.ext), or may have a path.
 * If the file name is just a base name, the path in a reference path name will be used.
 * @param {string} fileName The base file name or path name.
 * @param {string} refPathName The reference path name to use if fileName is just a base file name. refPathName
 * is presumed to include a base file name, that base file name will be replaced.
 */
...
/**
 * Interface for the file actions. File actions perform an action on a file, which can be reverted
 * until the action is finalized.
 * <p>
 * A set of file actions are normally created, and then passed to {@link performFileActions}. performFileActions()
 * handles calling {@link FileAction#apply}, {@link FileAction#finalize}, and {@link FileAction#revert} when appropriate.
 * @interface
 */
export class FileAction {
     ...
     * @param {string} backupFileName   The backupFileName arg that was passed to the {@link FileAction#setBackupFileName} call.
     * @param {string} backupPathName   The full path name of the created backup file.
     */

    /**
     * Sets a backup file name. If a file with {@link FileAction#getPathName} exists when the action is applied and a backup file name
     * has been specified, the existing file is renamed to the backup file name.
     * @param {string} backupFileName The optional file name for the backup file. Only the base name (name.ext) is used.
     * @param {FileAction~BackupCallback}   [callback]  Optional callback function that's called when the backup file is created.
     */
    ...
    async _applyForExistingFile() {
        if (await fileExists(this._pathName)) {
            if (this._backupFileName) {
                this._backupPathName = getFullPathName(this._backupFileName, this._pathName);
                this._backupAction = new RenameFileAction(this._pathName, this._backupPathName);
                await this._backupAction.apply();

                if (this._backupFileCallback) {
                    this._backupFileCallback(this._backupFileName, this._backupPathName);
                }
            }
    ...
    /**
     * Performs the action. This is where the bulk of the action should be performed, particularly
     * the parts where errors a reasonably likely to occur.
     * The action is not finalized until {@link FileAction#finalize} is called. Before then, the
     * action can be reverted by calling {@link FileAction#revert}
     */
    async apply() {
        try {
    ...
    /**
     * Finalizes the action. After this is called the action normally cannot be reverted.
     */
    async finalize() {
        try {
    ...
    /**
     * Reverts the action. This is normally called when an exception occurs before finalization.
...
/**
 * File action that deletes a file. The file to be deleted is temporarily renamed and is only deleted
 * when finalized. If the file does not exist this does not do anything.
 * @class
 */
...
export class RenameFileAction extends FileAction {
     ...
     * @param {string} newFileName The base file name or full path name of the renamed file.
     */
...
/**
 * File action for replacing an existing file or creating a new file (i.e. what happens when the original file doesn't exist).
 * The replacement is done via a callback function.
 * @class
 */
export class ReplaceFileAction extends FileAction {
    /**
     * Callback called by {@link ReplaceFileAction#apply}, {@link ReplaceFileAction#finalize}, or {@link ReplaceFileAction#revert} to handle
     * the actual creation of the file, extra finalization, or reversion.
     ...
     * @returns {Promise}   A promise should be returned, the callbacks are treated as <code>async</code> functions.
     */

    /**
     * @typedef {Object} ReplaceFileAction~Callbacks
     * @property {ReplaceFileAction~Callback} [applyCallback]   The callback to be called during {@link ReplaceFileAction#apply}.
     * @property {ReplaceFileAction~Callback} [finalizeCallback]    The callback to be called during {@link ReplaceFileAction#finalize}.
     * @property {ReplaceFileAction~Callback} [revertCallback]  The callback to be called during {@link ReplaceFileAction#revert}.
     */

    /**
     * @constructor
     * @param {string} pathName The full path name of the file to be created.
     * @param {ReplaceFileAction~Callback|ReplaceFileAction~Callbacks} callback Either a single {@link ReplaceFileAction~Callback}, in which
     * case it is the callback to be called during {@link ReplaceFileAction#apply}, or a {@link ReplaceFileAction~Callbacks} object containing
    ...
    async _applyMainAction() {
        ...
        if (this._noFileBackupFileName && !this._backupPathName) {
            if (await fileExists(this._pathName)) {
                const noFileBackupPathName = getFullPathName(this._noFileBackupFileName, this._pathName);
                ...
                if (this._noFileBackupFileCallback) {
                    this._noFileBackupFileCallback(this._noFileBackupFileName, noFileBackupPathName);
                }
            }
        }
    }
...
/**
 * File action that keeps an existing file. If {@FileAction~setBackupFileName} is called with a file name before the
 * action is applied, and the file exists, the existing file will be copied to the backup file name (actually, the existing
 * file is renamed to the backup file name, and then a copy of the backup file is made with the original name).
 * If the file does not exist or no backup file name has been set nothing happens.
 */
...
export class CopyFileAction extends FileAction {
     ...
     * @param {string} newFileName The name of the copy of originalFileName. One or both of originalFileName and newFileName
...
/**
 * Applies and finalizes all the {@FileAction}s in an array of file actions, calling {@link FileAction#revert} on
 * exceptions.
 * <p>
 * {@link FileAction#apply} is called in the order of appearance of the file actions in the list.
 * {@link FileAction#finalize} and {@link FileAction#revert} are called in the reverse order of appearance
 * in the list.
