...
/**
 * Simple parser of a decimal integer that returns <code>undefined</code> if the text contains any invalid characters.
 * @param {string} text The text to parse.
 * @returns {(number|undefined)}  The parsed integer or <code>undefined</code> if the text contained any invalid characters.
 * The only valid characters are optional leading whitespace, followed by an optional '-' sign, and then
...
/**
 * Makes sure a date can be compared with other backup dates. Basically strips off the time info.
...
/**
 * Default function for generating the name for a backup given the file name and a date.
 * @param {string} fileName The name of the file to be backed up. If the name contains a path the path will be kept.
...
export function defMakeBackupFileName(fileName, date, prefix) {
    ...
    parts.base = prefix + yearFormatter.format(date.getFullYear()) + monthFormatter.format(date.getMonth() + 1) + dayFormatter.format(date.getDate())
        + NAME_SEP + parts.base;
...
/**
 * Default funciton for parsing a backup file name into the original file name and the date.
 * @param {string} fileName The file name to parse.
 * @returns {(FileBackup~NameParts|undefined)}  The backup parts or <code>undefined</code> if the
 * file name is not a recognizable backup file name.
 */
...
/**
 * A class for creating dated backups of existing files when they are modified via {@link FileAction}s and restoring them,
 * also via {@link FileAction}s.
 * <p>
 * Note that the backup system currently requires all files be in the same directory.
 * <p>
 * To perform a backup, you first build an array of the {@link FileAction}s that will be performed to modify the files of
 * interest. You then pass this array on to {@link FileBackups#applyToFileActions}.
 * <p>
 * You can then apply the file actions. When the file actions are applied, the appropriate backup files are created.
 * <p>
 * Backup files are created by renaming the original files with prefix constructed from the backup date, of the form
 * 'BAK_20191011', which would represent a backup for Octobler 11, 2019.
 * <p>
 * If for some reason the file actions are reverted, the new backup files will also be reverted.
 * <p>
 * When it comes time to restore a backup, you call {@link FileBackups#getBackups} to obtain a list of the available
 * backups. You can then perform the actual restore via {@link FileBackups#restoreBackup} or obtain a list of {@link FileAction}s
 * to perform the backup later via {@link FileBackups#generateRestoreBackupFileActions}.
 * <p>
 * Backups are only made so there is one backup set for a given date. For a given date only the latest backup is kept.
 * <p>
 * The default maximum number of backups is 5, the {@link FileBackups~Options} object passed to the constructor can be used
 * to change that. Only the newest backups are kept. Old backups are automatically removed via actions added to the file action
 * list in the call to {@link FileBackups#applyToFileActions}.
 * @class
 */
export class FileBackups {
     ...
     * @property {number}   [maxBackups=5]  The maximum number of backups to support, 0 disables backups.
     * @property {Function} [makeBackupFileName={@link defMakeBackupFileName}]  The function for generating the backup file name.
     * @property {Function} [parseBackupFileName={@link defParseBackupFileName}]    The function for parsing the backup file name.
     */
     ...
     * @param {number} maxBackups The maximum number of backups. Existing available backups are not automatically adjusted.
     */
    setMaxBackups(maxBackups) { this._maxBackups = maxBackups; }


    /**
     * The main backup generation and management method. This utilizes an array of {@link FileAction}s representing the
     * actions that will modify the set of files for backing up.
     * <p>
     * If a backup is to be generated, {@link FileAction~setBackupFileName} will be called for each file action with an appropriate
     * backup file name.
     * <p>
     * Additionally, if there are more backups available than {@link FileBackups~getMaxBackups}, {@link DeleteFileAction}s will
     * be added to the file action list.
     * @param {FileAction[]} fileActions The original list of file actions. Actions for cleaning up existing backups may be added to
     * this list.
     * @param {Date} [backupDate=new Date()]    Optional date for the backup, intended only for testing.
    ...
    async applyToFileActions(fileActions, backupDate) {
        ...
        if (dir) {
            // Check for existing backup files, we want to get rid of those before we do anything else.
            ...
            const cleanupActions = await this.generateCleanupBackupFileActions(dir, date);
     ...
     * @property {string}   originalFileName    The base file name (name.ext) of the original file.
     */

    /**
     * @typedef {object} FileBackups~BackupSet
     * @property {Date} date    The date of the backup set.
     * @property {FileBackups~FileNames[]}  fileNames An array containing the file names for each backup file in the backup set.
     */
     ...
     * @returns {FileBackups~BackupSet[]}   An array of backup sets. The array is sorted with the newest backup sets
     * at lower indices.
     */
    ...
    /**
     * Generates a list of {@link FileActions}s needed to restore a backup set. The backup set should have
     * been one returned by {@link FileBackups#getBackups}.
     * @param {FileBackups~BackupSet} backupSet The backup set to be restored.
     * @param {string[]}    [governedFiles] Optional array of files which are governed by the backups. If a file is not being
     * replaced by a backup file, a delete action will be added. These should be base file names (name.ext)
    ...
    async restoreBackup(backupSet, existingFileNames) {
        const fileActions = await this.generateRestoreBackupFileActions(backupSet, existingFileNames);
        await performFileActions(fileActions);
    }


    /**
     * Generates {@link FileAction}s for deleting the files of a backup set. The backup set should have
     * been one returned by {@link FileBackups#getBackups}.
     * @param {FileBackups~BackupSet} backupSet The backup set to be deleted.
     * @returns {FileAction[]} The array of file actions to be performed to remove the backup set.
     */
    async generateDeleteBackupFileActions(backupSet) {
        const fileActions = [];
        const { fileNames } = backupSet;
        fileNames.forEach((fileNamePair) => fileActions.push(new DeleteFileAction(fileNamePair.pathName)));
        return fileActions;
    }
    ...
    /**
     * Generates {@link FileAction}s as needed for deleting backups that exceed the backup limits.
     * @param {string} dir The directory containing the backups.
     * @returns {FileAction[]}  The array of file actions to be performed for cleaning up.
     */
    async generateCleanupBackupFileActions(dir, currentBackupDate) {
        ...
        if ((this._maxBackups > 0) && dir) {
            ...
            // If a current backup date is given, we want to remove that backup from the backup list.
            ...
            for (let i = backups.length - 1; i >= endBackupIndex; --i) {
                ...
                fileActions = fileActions.concat(await this.generateDeleteBackupFileActions(backups[i]));
            }
        }
    ...
    /**
     * Helper for cleaning up extra backup sets. Normally the cleanup is done via extra {@link FileAction}s being
     * added to {@link FileBackups#applyToFileActions}.
     * @param {string} dir The directory containing the backups.
     */
